...
d3.json("data/polioData.json", function (preVac){
	preVac.splice(0, 266)
	preVac.splice(12, preVac.length -1)
	console.log(preVac)
	var clean_data = JSON.stringify(preVac);
	$('#poxDataGraph').text(clean_data)
})
d3.json("data/measlesPreVac.json", function (fAll){
	d3.json("data/measlesPostVac.json", function (pAll){
		//sliderCircles(fAll[0])
		//spiderCircles(pAll[0])
		$('#slider').on('click', function (){
			sliderCircles(fAll[this.value])
			spiderCircles(pAll[this.value])
		})
	});
});
var spiderCircles = function (input) {
	//remover previous circles//
	svg1.selectAll('.maps').remove();
	svg1.selectAll("circle")
		...
		.each("end", function(){
			...
			.attr("r", function(d){ 
				if(d.location !== 'total') {
					return Math.sqrt(parseInt(d.cases) * 10)
				}
			})
			.style('opacity', '.7')
		})
...
var sliderCircles = function (input) {
	//remover previous circles//
	svg2.selectAll('.map').remove();
	svg2.selectAll("circle")
		.data(input)
		.enter()
		.append("circle")
		.attr('class', 'map')
		.attr('fill','rgba(183,28,28, 0.7')
		.style('stroke', '#fff')
		.style('stroke-width', 1)
		.transition()
		.duration(10)
...
//makes chloropleth map/////
var svg2, svg1, projection, map, xAxis;
...
var  drawclearMap = function(json) {
	var color = d3.scale.quantize();
		color.range(["#E1BEE7", "#BA68C8", "#AB47BC", "#9C27B0","#8E24AA","#7B1FA2","#6A1B9A","#4A148C","#EA80FC","#E040FB","#D500F9","#AA00FF"]);
		color.domain([0, 660])
		//get map object//
...
// d3.json("data/measlesPreVac.json", function (pre){
	...
	// for(var i = 0; i < pre.length; i++) {
		// var date = pre[i].month_yr.slice(0,3)
		// if(date === "Oct" || date === 'Nov' || date === 'Dec') {
			// sum += parseInt(pre[i].cases, 10)
		// }
	// }
	// console.log(sum/len)
// })
//makes compare data for measles//
var makedata = function () {
	...
	d3.json("data/measlesPreVac.json", function (pre){
		d3.json("data/measlesPostVac.json", function (post){
			...
			var date = pre[i].month_yr.slice(0,3)
			if(date === "Oct" || date === 'Nov' || date === 'Dec') {
				compAll.push([{ label:"Pre-Vaccine Fall", year: pre[i].month_yr ,disease: pre[i].disease, time: pre[i].time, lat: "38.349" , lon: "-111.893" ,cases: '2747.85'},{ label:"Post-Vaccine Fall",year: post[i].month_yr ,disease: post[i].disease, time: post[i].time, lat: "38.349" , lon: "-81.683" ,cases: post[i].cases}])
				}
			}
			//console.log(compAll)
			//var clean_data = JSON.stringify(compAll);
			//d3.select('#print').text(clean_data);
...
//get averange casses//
d3.json("data/polioPreVac.json", function (pre){
	//console.log(pre)
	var len = pre.length;
	var sum = 0;
	for(var i = 0; i < pre.length; i++) {
		var date = pre[i].month_yr.slice(0,3)
		if(date === "Jul" || date === 'Aug' || date === 'Sep') {
			sum += parseInt(pre[i].cases, 10)
		}
	}
	console.log(sum/len)
...
//makes data for comparing polio///
d3.json("data/polioPreVac.json", function (pre){
	d3.json("data/polioPostVac.json", function (post){
			var compAll = [];
			for(var i = 0; i < pre.length; i++) {
			var date = pre[i].month_yr.slice(0,3)
		if(date === "Jul" || date === 'Aug' || date === 'Sep') {
				compAll.push([{ label:"Pre-Vaccine Summer", year: pre[i].month_yr ,disease: pre[i].disease, time: pre[i].time, lat: "38.349" , lon: "-111.893" ,cases: '1527.46'},{ label:"Post-Vaccine Summer",year: post[i].month_yr ,disease: post[i].disease, time: post[i].time, lat: "38.349" , lon: "-81.683" ,cases: post[i].cases}])
				}
			}
			//console.log(compAll)
			//var clean_data = JSON.stringify(compAll);
...
d3.json("data/polioPreVac.json", function(pre) {
	d3.json("data/polioPostVac.json", function(post) {
		...
		var bucket = [];
			for (var i = 0; i < 50; i++) {
				bucket.push({cases: pre[j][i].cases, postcases: post[j][i].cases, month_yr: pre[j][i].month_yr , location: pre[j][i].location, disease: "Polio Comparison", lat: pre[j][i].lat, lon: pre[j][i].lon, time: pre[j][i].time })
			}
				data.push(bucket)
		}
		var clean_data = JSON.stringify(data);
		d3.select('#print').text(clean_data);
	});
});