...
var runMeaslesCompare = function(pre, msAll) {
	...
	var compareMeasles = function () {
		...
		$('.btn-container').append("<button id='measles-return' class='btn btn-return'>Return to Measles</button>");
		d3.select('#cases-one').style("color", "#9eda62");
		d3.select('#cases-two').style("color", "#81FC06");
...
var runPolioCompare = function(pre, pAll) {
	...
	var comparePolio = function () {
		...
		d3.select('.stats-holder').classed('hidden', true);
		d3.select('.stats-holder-two').classed('hidden', false);
		$('.btn-container').append("<button id='polio-return' class='btn btn-return'>Return to Polo</button>")
...
var animateChickenPox = function(pxAll) {
	svg2.selectAll('circle').remove();
	var num = 0;
	clearAnimationButtons()
	...
	var max = 219;
		...
		sldr.value = num;
		var color = "percs"
		anCircles(pxAll[num], color);
		seasons(pxAll[num]);
		labels(pxAll[num], num)
			num++;
...
var animateMeasles = function(msAll) {
	svg2.selectAll('circle').remove();
	var num = 0;
	clearAnimationButtons();
	...
	var max = 866;
		intevalId = setInterval(function(){
		var sldr = document.getElementById('sldr-two')
		sldr.value = num;
		var color = "mercs"
		anCircles(msAll[num], color);
		seasons(msAll[num]);
		labels(msAll[num], num)
			num++;
