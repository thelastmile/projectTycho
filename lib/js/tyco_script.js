...
var runMeaslesCompare = function(pre, msAll) {
	...
	var compareMeasles = function () {
		...
		$('#measles-return').on('click', function(){
			...
			seasons(msAll[0]);
			$('.btn-container').append("<button id='measles-animate' class='btn btn-return'>Watch Over Time</button>")
		 		d3.select('#measles-animate').on('click', function(){
		 	$('.btn-container').append("<button id='measles-stop-animate' class='btn btn-return'>Stop</button>");
...
var runPolioCompare = function(pre, pAll) {
	...
	var comparePolio = function () {
		...
		$('#polio-return').on('click', function(){
			...
			seasons(pAll[0]);
			$('.btn-container').append("<button id='polio-animate' class='btn btn-return'>Watch Over Time</button>")
		 		d3.select('#polio-animate').on('click', function(){
		 	$('.btn-container').append("<button id='polio-stop-animate' class='btn btn-return'>Stop</button>");
...
var runPox = function (pxAll) {
	d3.select('#pox-page').on('click', function () {
		...
		seasons(pxAll[0]);
		clearAnimationButtons();
		$('.btn-container').append("<button id='chickenpox-animate' class='btn btn-return'>Watch Over Time</button>")
	 
	 d3.select('#chickenpox-animate').on('click', function(){
	 	$('.btn-container').append("<button id='chickenpox-stop-animate' class='btn btn-return'>Stop</button>");
...
var runFlu = function (fAll) {
	d3.select('#flu-page').on('click', function () {
		...
		seasons(fAll[0]);
		clearAnimationButtons();
		$('.btn-container').append("<button id='flu-animate' class='btn btn-return'>Watch Over Time</button>")
	 			d3.select('#flu-animate').on('click', function(){
	 		$('.btn-container').append("<button id='flu-stop-animate' class='btn btn-return'>Stop</button>");
...
var chikenPox =  function () {
	...
	d3.select('#mid-txt').text('No data availabe 1982-2006');
	displays();
};
...
var flu =  function () {
	clearAll();
	var years = [1927, 1930, 1933, 1935, 1938, 1941, 1943, 1945, 1949, 1951];
	var range = years.length;
	addAxis(years, range);
	hideSliders();
	$('#sldr-four').show();
